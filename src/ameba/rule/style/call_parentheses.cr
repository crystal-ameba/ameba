module Ameba::Rule::Style
  ...
  class CallParentheses < Base
    ...
    def test(source, node : Crystal::Call)
      ...
      if heredoc_arg = find_heredoc_arg(node, source)
        return if exclude_heredocs?
      end
      ...
      location, end_location =
        call_locations(node, heredoc_arg, source)
    ...
    # Returns the start and end locations of a call *node*.
    #
    ...
    #     end
    #
    private def call_locations(node, heredoc_arg, source)
      location = name_end_location(node).try(&.adjust(column_number: 1))

      case
      when block = node.block
        end_location = if short_block?(block, source)
    ...
    private def find_heredoc_arg(node : Crystal::Call, source)
      node.args.find do |arg|
        find_heredoc_arg(arg, source)
      end
    end
