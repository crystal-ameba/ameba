module Ameba::Rule::Lint
  ...
  class DuplicateBranch < Base
    ...
    def test(source, node : Crystal::If | Crystal::Unless | Crystal::Case | Crystal::ExceptionHandler, ifs : Enumerable(Crystal::If)? = nil)
      processed_bodies = Set(String).new
      ...
      each_branch(ifs || node) do |body_node|
        ...
        body_s = body_node.to_s

        if processed_bodies.includes?(body_s)
          issue_for body_node, MSG
        else
          processed_bodies << body_s
        end
      end
    end
