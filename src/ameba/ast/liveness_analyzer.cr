module Ameba::AST
  ...
  class LivenessAnalyzer
    ...
    # Crystal dispatches to the most specific matching overload at runtime
    # when the argument is a virtual type (Crystal::ASTNode+).
    ...
    private def propagate_through(node : Crystal::If, live : LiveSet, mark = true) : LiveSet
    ...
    private def propagate_through(node : Crystal::Unless, live : LiveSet, mark = true) : LiveSet
      then_live = propagate_through(node.then, live, mark)
      else_live = propagate_through(node.else, live, mark)
      merged = then_live | else_live
      propagate_through(node.cond, merged, mark)
    ...
    private def propagate_through(node : Crystal::While, live : LiveSet, mark = true) : LiveSet
      propagate_through_loop(node.cond, node.body, live, mark)
    end

    private def propagate_through(node : Crystal::Until, live : LiveSet, mark = true) : LiveSet
      propagate_through_loop(node.cond, node.body, live, mark)
    end

    private def propagate_through(node : Crystal::Case, live : LiveSet, mark = true) : LiveSet
      propagate_through_case(node, live, mark)
    end

    private def propagate_through(node : Crystal::Select, live : LiveSet, mark = true) : LiveSet
      propagate_through_case(node, live, mark)
    end
    ...
    {% for type in %w[Block Def ProcLiteral ClassDef ModuleDef EnumDef
    ...
    # Fallback: generic child propagation for nodes not specifically handled
    ...
    # --- Shared helpers for types with identical logic ---
